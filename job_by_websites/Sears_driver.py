...
def do_the_job(jobdict, driver=None):
    ...
    'JOB': {'SKU': None,
            'UPC': None,
            'availability': ['''//header[contains(text(), 'Delivery Options')]/parent::div''',
                             '''js(return document.evaluate("//*[@id='shipping']/div/p", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.textContent.trim();)''',
                             '''//figure[contains(@class,'FulfillmentShipPanel') and contains(@style,'display: block')]/span[contains(@class,'fulfillmentNav')]'''
                            ],
            'brand': '//th[text()="Brand"]/parent::tr/td[@class="value font-semibold"]',
            'channel': '//a[@class="font-bold prod-SoldShipByMsg"]',
            'meta': {'sales' : '''//div[@class='productOptionPrices']//*[@class='salePrice']''',
                     'regular':'''//div[@class='productOptionPrices']//h3[contains(@class,'defaultPrice')]'''},
            'model': None,
            'page_path': '//div[contains(@id,"breadcrumb")]//ul[contains(@class,"breadcrumbs")]',
            'price': [
                #'''//div[@class='productOptionPrices']//*[@class='salePrice']''',
                '''//meta[@itemprop="price"]/@content''',
                      '//div[@id="gnf_content"]//li[contains(@class,"product-price-big")]//span[contains(@class,"price-wrapper")]',
                '''js(return document.evaluate("//div[@id='gnf_content']//li[contains(@class,'product-price-big')]//span[contains(@class,'price-wrapper')]", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.textContent)'''
                      ],
            'product_title': [
                '''//meta[@property="og:title"]/@content''',
                '//div[@id="gnf_content"]//header[contains(@class,"product-header")]'],
    ...
    'PRE_SETUP': [{'actioncode': 'click',
                   'target': '''//figure[not(contains(@style,'display: none'))]//a[@data-method="delivery" and @href]|//figure[contains(@class,'panel-fulfillment') and contains(@class,'ShipPanel') and contains(@style,'display: block')]'''},
                  {'actioncode': 'clear',
                   'target': "//figcaption[@style]//div[contains(@class,'panel-content-scroll')]//input[@placeholder = 'Enter Location']"},
                  {'actioncode': 'send_keys',
                   'target': "//figcaption[@style]//div[contains(@class,'panel-content-scroll')]//input[@placeholder = 'Enter Location']",
                   'value': '12603'},
                  '''js(document.getElementById('L1V2').setAttribute('class','hide');)''',
                  {'actioncode': 'click',
                   'target': '''//figcaption[@style]//div[contains(@class,'panel-content-scroll')]//button[@type='submit']'''},
                  {'actioncode' : 'sleep',
                   'value':'1'},
                  {'actioncode' : 'click',
                   'target':'''//button[@class='button-primary' and @data-id = 'addToCart']'''},
                  {'actioncode': 'sleep',
                   'value': '3'}
                  ],
    'PRE_REQ': '''//header[contains(text(), 'Delivery Options')]/parent::div''',
    'STATUS': 'READY',
        'URL': 'http://www.sears.com/x/p-{}'.format(jobdict['PID']) if jobdict['URL'] is None else jobdict[
            'URL'],
        'WEBSITE': 'Sears'}
...
if __name__ == '__main__':
    payload_dict = {
        'WEBSITE': 'Sears',
        'PID': '02214573000P',#'02294203000P',#,
        'SKU': '02214573000P',#'02294203000P',#'02010340000P',
        'URL': None,
        'TAG':'TEST'
