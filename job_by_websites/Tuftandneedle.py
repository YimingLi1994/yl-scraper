                ...
                for eachitem in thisitemlst:
                    ...
                    if len(jsstrlst) > 0:
                        ...
                        try:
                            elem = driver.find_element_by_xpath(eachitem)
                            jobretdir[key] = elem.text
                            if len(jobretdir[key]) == 0:
                                jobretdir[key] = elem.get_attribute('innerText')
                        except InvalidSelectorException as e:
                            try:
                                jobretdir[key] = driver.find_element_by_xpath('/'.join(eachitem.split('/')[:-1])) \
                                    .get_attribute(eachitem.split('/')[-1][1:])
...
def do_the_job(jobdict, driver=None):
    workload = {'CONTENT_REQ': ['''//h4[@data-id="price"]'''],
                'JOB': {'SKU': None,
                        'UPC': None,
                        'brand': None,
                        'model': '''//ul[@class="products" and @data-original-price="true"]//label[@class='active']''',
                        'page_path': None,
                        'price': '''//h4[@data-id="price"]''',
                        'meta': None,
                        'product_title': ['''//div[@id="product_select"]/h3''','''//div[@class="exp_shipping_one"]/h3''']},
                'PID': jobdict['PID'],
                'STATUS': 'READY',
                'URL': jobdict['URL'],
                'WEBSITE': 'Tuftandneedle'}

    ret_dict = work_on_the_job(workload, driver)
    ret_dict['SKU'] = jobdict['SKU']
    ret_dict['price'] = ret_dict['price'].replace("$", '')
    ret_dict['model'] = ret_dict['model'].split("$")[0]
...
if __name__ == '__main__':
    payload_dict = {
        'WEBSITE': 'Tuftandneedle',
        'PID': 'MAT-MNT-02-T',#'MAT-MNT-01-T',
        'SKU': None,
        'URL': 'https://www.tuftandneedle.com/mattress/mint/?size=MAT-MNT-02-T',#'https://www.tuftandneedle.com/mattress/mint/?size=MAT-MNT-01-T',
        'TAG': 'mattress'
    }
