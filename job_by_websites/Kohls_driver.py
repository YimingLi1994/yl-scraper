...
def do_the_job(jobdict, driver=None):
    workload = {
        'JOB': {
            'SKU': ['''js(return productV2JsonData["SKUS"][0]['skuCode'])'''],
            'UPC': ['''js(return productV2JsonData['SKUS'][0]['UPC']["ID"])'''],
            'availability': ['''js(return  productV2JsonData['SKUS'][0]['availability'])''',
                            '//meta[@itemprop="availability"]/@content'],

            'brand': ['//meta[@itemprop="brand"]/@content',
                     '''js(return productV2JsonData['monetization']['brand']['value']])'''],

            'channel': None,
            'meta': None,
            'model': ['''js(return productV2JsonData['accordions']['productDetails'])'''],
            'page_path':['''//div[contains(@class, "pdp_breadcrumb")]'''],
            'price': ['//div[contains(@class, "main-price ")]',
                      '//div[@class="regorg-small"]' ],

            'price_type': ['//div[contains(@class,"main-pricelabel") ]'],


            'product_title': ['//h1[@class="pdp-product-title"]',
                              '//div[@class="pdp_outofstockproduct"]'],
            'rating': '''js(return  productV2JsonData['avgRating'])''',
            'reviews': '''js(return  productV2JsonData['ratingCount'])''',
            'shipping': None,
            'style': '''js(return productV2JsonData["SKUS"][0]['color'])''',
            'style2':'''js(return productV2JsonData["SKUS"][0]['size'])'''
        },
        'PID': jobdict['PID'],
        'STATUS': 'READY',
        'URL': jobdict['URL'],
...
if __name__ == '__main__':
    payload_dict = {"WEBSITE": "Kohls",
                     "PID": "3007881",
                    "SKU": "null",
                    "URL": "http://www.kohls.com/product/prd-3007881/bissell-pet-hair-eraser-lift-off-upright-vacuum.jsp?prdPV=2",
                    "TAG": 'test'}
