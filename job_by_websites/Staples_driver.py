...
def do_the_job(jobdict, driver=None):
    workload = {'CONTENT_REQ': None,
                'JOB': {'SKU': None,
                        'UPC': "js(return jsonPath(STAPLES.angularData, '$..upc_code')[0])",
                        'availability': [
                            "js(if(jsonPath(STAPLES.angularData, '$..noninventory_flag')[0]===0){return 'IN_STOCK'}else{return 'OUT_OF_STOCK'})",
                            '//div[@ng-controller="prodInfo"]//div[@id="getPrice"]/div[@class="for-skuset"]/div[contains(@ng-hide,"product.metadata.product_type") and not(contains(@class,"ng-hide"))]//div[contains(@class,"preferred-price-wrapper")]/div[not(contains(@class,"ng-hide"))]/div[contains(@ng-show,"isDelivery")]//div[@class="delivery"]/div[@class="shipping-info"]/div[contains(@ng-if,"DeliveryInfo")]/div[@ng-show and not(contains(@class,"ng-hide"))]',
                            '//p[@class="checkOutFont"]'],
                        'brand': None,
                        'channel': None,
                        'meta': '//div[contains(@class,"prod-detail") and contains(@class,"specification")]/div[contains(@class,"prod-spec-table")]',
                        'model': ["js(return jsonPath(STAPLES.angularData, '$..mfpartnumber')[0])",
                                  '//div[contains(@class,"product-title")]//li[contains(.,"Model") or contains(.,"model")]'],
                        'page_path': '//*[@id="mainNgApp"]//div[contains(@class,"breadcrumbs")]/ul/li[@typeof]/parent::ul',
                        'price': ["js(return jsonPath(STAPLES.angularData, '$..finalPrice')[0])",
                                  '//div[@ng-controller="prodInfo"]//div[@id="getPrice"]/div[@class="for-skuset"]/div[contains(@ng-hide,"product.metadata.product_type") and not(contains(@class,"ng-hide"))]//div[contains(@class,"preferred-price-wrapper")]/div[not(contains(@class,"ng-hide"))]//div[contains(@scvalue,"delivery") and not(@ng-hide)]//div[@class="price"]//span[contains(@class,"preferred-price") and not(contains(@class,"ng-hide"))]'],
                        'price_type': "js(if(jsonPath(STAPLES.angularData,'$..price_in_cart_flag')[0]===1)\n{return 'IN CART';} else if(jsonPath(STAPLES.angularData,'$..clearance_candidate_flag')[0]===1){return 'CLEARANCE_CANDIDATE';} \nelse if(jsonPath(STAPLES.angularData,'$..clearance_manual_flag')[0]===1){return 'CLEARANCE_manual';})",
                        'product_title': [
                            '//div[@ng-controller="prodInfo"]//div[contains(@class,"grid")]/h1[contains(@ng-bind-html,"product") and contains(@ng-bind-html,"name")]',
                            '//p[@class="checkOutFont"]',
                            '//div[@class="results-title"]',
                            '//div[@class="it-title"]/h1',
                            """js(return jsonPath(STAPLES.angularData,'$..metadata.name')[0])"""],
                        'rating': '//div[contains(@class,"product-title")]//*[contains(@class, "subtitle")]//span[contains(@class,"Review")]/span/@title',
                        'reviews': '//div[contains(@class,"product-title")]//*[contains(@class, "subtitle")]//span[contains(@class,"Review")]/parent::div/span[contains(.,"Write a Review") and not(contains(@class,"ng-hide"))]',
                        'shipping': None,
                        'style': '//div[@ng-controller="prodInfo"]//div[@id="getPrice"]/div[@class="for-skuset"]/div[not(contains(@class,"ng-hide"))]/div[@ng-switch]//div[contains(@class, "value")]',
                        'style2': None},
                'PID': jobdict['PID'],
                'PRE_REQ': "js(return typeof jsonPath ==='function')",
                'PRE_SETUP': [
                    "js(var script = document.createElement('script');script.type = 'text/javascript'; script.src = 'https://storage.googleapis.com/jx-cdn/jsonpath-0.8.0.js'; document.head.appendChild(script);)"],
                'STATUS': 'READY',
                'URL': 'http://www.staples.com/-/product_{}'.format(jobdict['PID']) if jobdict['URL'] is None else
                jobdict['URL'],
                'WEBSITE': 'Staples'}
...
if __name__ == '__main__':
    payload_dict = {"WEBSITE": "Staples",
                    "PID": "1035219",
                    "SKU": "1035219",
                    "URL": "https://www.staples.com/staples-glenvar-bonded-leather-big-and-tall-chair/product_1035219",
                    "TAG": 'test'}
