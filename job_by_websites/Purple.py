...
def do_the_job(jobdict, driver=None):
    if jobdict['PID'] == "purple-mattress":
        type_lst = ['''//a[@data-type="Purple.2"]''', '''//a[@data-type="Purple.3"]''','''//a[@data-type="Purple.4"]''']
    else:
        type_lst = ['''|"Purple.Original"|''']
    size_lst = ['''//a[@data-size="Twin XL"]''','''//a[@data-size="Full"]''','''//a[@data-size="Queen"]''','''//a[@data-size="King"]''','''//a[@data-size="Cal King"]''']
    ret_dict_total = []
    for mat_type in type_lst:
        for mat_size in size_lst:
            workload = {
                'CONTENT_REQ': ['''//div[@class="price-wrapper"]'''],
                'JOB': {
                    'SKU': None,
                    'UPC': None,
                    'brand': None,
                    'meta': None,
                    'model': None,
                    'price': ['''//div[@class='price-wrapper']//span[@class="price"]'''],
                    'price_type': None,
                    'product_title': '''//section[@class="new-mattress-subnav"]//div[@class="container"]/span''',},
                ...
                'PID': jobdict['PID'],
                'STATUS': 'READY',
                'URL':   'https://purple.com/mattresses/{}/buy'.format(jobdict['PID']) if jobdict['URL'] is None else jobdict['URL'],
                'WEBSITE': 'Purple'}
            ret_dict = work_on_the_job(workload, driver)
            ret_dict_total.append(ret_dict)
            time.sleep(3)
    print(ret_dict_total)
    i=0
    for mat_type in type_lst:
        for mat_size in size_lst:
            ret_dict_total[i]['model'] = mat_size.split('"')[1]
            ret_dict_total[i]['product_title'] = mat_type.split('"')[1]
            ret_dict_total[i]['price'] =  ret_dict_total[i]['price'].replace(",",'')
            i+=1

    #ret_dict['SKU'] = jobdict['SKU']
    # if False: #ret_dict['product_title'] is None:
...
if __name__ == '__main__':
    payload_dict = {
        ...
        'PID': 'purple-bed',
        'SKU': None,
        'URL': 'https://purple.com/mattresses/purple-bed/buy', #'https://purple.com/mattresses/purple-bed/buy',
        'TAG': 'mattress'
    }
