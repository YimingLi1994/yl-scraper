...
def do_the_job(jobdict, driver=None):
    workload = {'CONTENT_REQ': ["//span[@class='fv-zip-code']/a[@href]"],
                'JOB': {'SKU': '//*[@id="sku-model"]//li[@class="sku-id"]',
                        'UPC': ['js(return JSON.parse(document.evaluate("//*[@id=\'pdp-content\']/script[contains(@type,\'json\')]", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.text)[\'gtin13\'];)',
                                '''js(return window.find_json()['gtin13'];)'''
                                ],
                        'availability': ['//*[@id="phoenix-availability"]',
                                         '//*[@id="pdp-add-to-cart-button"]//button[1]',
                                         '//*[@id="pdp-inactive-content"]//div[contains(@class,\'alert\')]'],
                        'brand': 'js(return document.evaluate("//*[@class=\'sku-title\']/h1", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.textContent.split(\'-\')[0].trim();)',
                        'channel': None,
                        'meta': {'Special Offers':"//strong[contains(text(),'Special Offers')]/parent::div", "Division":'''js(var ol = document.querySelectorAll('ol[id="breadcrumb-list"]')[0]; return ol.children[1].children[0].text)'''},
                        'model': ['//*[@id="sku-model"]//li[@class="model-number"]',
                                  '//*[@id="model-value"]',
                                  '''js(return window.find_json()['model'];)'''],
                        'page_path': '//*[@id="breadcrumb-list"]',
                        'price': ["js(return window.pdp.priceBlock['price']['currentPrice'])",
                                  '//*[@id="priceblock-wrapper-wrapper"]//div[contains(@class,"pb-price")]/div[contains(@class,"pb-hero-price")]/span'],
                        'price_type': "js(return window.pdp.priceBlock['price']['pricingType'])",
                        'product_title': ['//*[@id="sku-title"]/h1',
                                          '//*[@id="pdp-inactive-content"]//div[contains(@class,\'alert\')]',
                                          '//*[@class="sku-title"]/h1'],
                        'rating': '//li[@class="customer-rating"]//span[contains(@class,"score")]',
                        'reviews': '//li[@class="customer-rating"]//span[not(@*)]',
                        'shipping': '//*[@id="phoenix-availability"]//span[contains(@class,"fv-price")]',
                        'style': '//*[@id="product-variations"]',
                        'style2': None},
                'PID': jobdict['PID'],
                'PRE_REQ': ['//*[@id="sku-title"]/h1|//*[@id="pdp-inactive-content"]//div[contains(@class,\'alert\')]',
                            '''js(return typeof window.find_json() !== 'undefined')'''],
                'PRE_SETUP': [
                    '''js(window.find_json = function() {var ldjsonlst=document.querySelectorAll('script[type="application/ld+json"]');for(var i=0; i<ldjsonlst.length; i++){    if(JSON.parse(ldjsonlst[i].text)['@type'] === 'Product'){        return JSON.parse(ldjsonlst[i].text);    }}};window.find_low_price = function(){    var offers = window.find_json()['offers'];    var price_list = [];    for(var i=0; i<offers.length; i++){        if(offers[i]['availability'].includes("InStock")){        price_list.push(offers[i]['price']);    }    return Math.min(price_list);    }};)''',
                            {'actioncode': 'click',
                               'target': "//span[@class='fv-zip-code']/a[@href]"},
                              {'actioncode': 'clear',
                               'target': "//form[@class='fv-zip-code__form']//input[@placeholder]"},
                              {'actioncode': 'send_keys',
                               'target': "//form[@class='fv-zip-code__form']//input[@placeholder]",
                               'value': '12603'},
                              {'actioncode': 'click',
                               'target': '//form[@class="fv-zip-code__form"]//button[contains(text(),\'Update\')]'}],
                'STATUS': 'READY',
                'URL': 'http://www.bestbuy.com/site/-/{}.p'.format(jobdict['PID']) if jobdict['URL'] is None else jobdict['URL'],
                'WEBSITE': 'BestBuy'}
