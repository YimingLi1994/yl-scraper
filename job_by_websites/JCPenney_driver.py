    ...
    workload = {'JOB':
        {
            'SKU': None,
            'UPC': None,
            'availability': '''js(if(window.find_low_price() > 0){return 'InStock';} else{return 'OOS';})''',
            'brand': '''js(return window.find_json()['brand']['name'])''',
            'channel': None,
            'meta': None,
            'model': ['//th[text()="Model"]/parent::tr/td[@class="value font-semibold"]',
                      '//meta[@itemprop="model"]/@content'],
            'page_path': '''js(var crumblist = window.find_crumb()['itemListElement'];retarray = [];for(var i=0; i<crumblist.length; i++){    retarray.push(crumblist[i]['item']['name']);}return retarray.join(' ~ ');)''',
            'price': '''js(return window.find_low_price())''',
            'price_type': '''//div[@data-automation-id="product-content-block"]//*[@data-automation-id="sale-price-info"]/span[@data-automation-id="at-price-label"]''',
            'product_title': ['''//div[@data-automation-id="product-title"]/h1''',
                              ],
            'rating': '''js(return window.find_json()['aggregateRating']['ratingValue'];)''',
            'reviews': '''js(return window.find_json()['aggregateRating']['reviewCount'];)''',
            'shipping': None,
            'style': None,
            'style2': None},
        'PID': jobdict['PID'],
        ...
        'PRE_SETUP': [
            ...
            '''js(window.find_crumb = function() {    var ldjsonlst=document.querySelectorAll('script[type="application/ld+json"]');    for(var i=0; i<ldjsonlst.length; i++){        if(JSON.parse(ldjsonlst[i].text)['@type'] === 'BreadcrumbList'){            return JSON.parse(ldjsonlst[i].text);        }    }    }; )'''
        ],
        'STATUS': 'READY',
        'URL': 'https://www.jcpenney.com/p/-/{}'.format(jobdict['PID']) if jobdict['URL'] is None else jobdict[
            'URL'],
        'WEBSITE': 'JCPenney'}
...
if __name__ == '__main__':
    payload_dict = {"WEBSITE": "JCPenney",
                    "PID": "ppr5007325777",
                    "SKU": None,
                    "URL": "https://www.jcpenney.com/p/step2-whisper-ride-cruiser-ride-on/ppr5007325777",
                    "TAG": 'test'}
