...
def do_the_job(jobdict, driver=None):
    workload = {'CONTENT_REQ': None,
                'JOB': {'SKU': "js(return jsonPath(targ,'$..product..firstSkuId')[0])",
                        'UPC': "js(return jsonPath(targ,'$..product..upCeaNisbN')[0])",
                        'availability': [
                            "js(if(jsonPath(targ,'$..productDetails..SKUsList')[0].length > 0){if(jsonPath(targ,'$..product..actionType')[0] == 'OUT_OF_STOCK'){return 'Out_Of_Stock';}else{return null;}}else {return null;})",
                            "js(if(jsonPath(targ,'$..productDetails..SKUsList')[0].length > 0){if(jsonPath(targ,'$..product..actionType')[0] == 'ADD_TO_CART'){return 'In_Stock';}else{return null;}}else {return null;})",
                            "js(if(jsonPath(targ,'$..productDetails..SKUsList')[0].length > 0){return jsonPath(targ,'$..product..actionType')[0]})",
                            "//div[@class='no-results-found__content']/h3"],
                        'brand': "js(return jsonPath(targ,'$..product..brandName')[0])",
                        'channel': None,
                        'meta': None,
                        'model': "js(return jsonPath(targ,'$..product..manufacturer')[0])",
                        'page_path': "js(var retstr=''; for(var i=0; i<jsonPath(targ, '$..breadcrumb')[0].length; i++){retstr+=jsonPath(targ, '$..breadcrumb')[0][i].name + '~';} return retstr;)",
                        'price': "js(return jsonPath(targ,'$..product..salePrice')[0])",
                        'price_type': None,
                        'product_title': ["//div[@class='pdp-header']//h1/@title",
                                          "//h2[@class='geoffrey-content-bellow__top-header']"],
                        'rating': "js(return jsonPath(targ,'$..product..reviewRating')[0])",
                        'reviews': "js(return jsonPath(targ,'$..product..reviews')[0])",
                        'shipping': "//span[@class='pdp-promos__promo-desc']",
                        'style': None,
                        'style2': None},
                'PID': jobdict['PID'],
                'PRE_REQ': "js(return typeof jsonPath ==='function')",
                'PRE_SETUP': "js(var script = document.createElement('script');script.type = 'text/javascript'; script.src = 'https://storage.googleapis.com/jx-cdn/jsonpath-0.8.0.js'; document.head.appendChild(script); targ = window.__INITIAL_STATE__;)",
                'STATUS': 'READY',
                'URL': 'https://www.toysrus.com/product?productId={}'.format(jobdict['PID']) if jobdict[
                                                                                                    'URL'] is None else
                jobdict[
                    'URL'],
                'WEBSITE': 'ToysRUs'}
...
if __name__ == '__main__':
    payload_dict = {"WEBSITE": "ToysRUs",
                    "PID": "92898836",
                    "SKU": "92898836",
                    "URL": "https://www.toysrus.com/product?productId=92898836",
                    "TAG": 'test'}
